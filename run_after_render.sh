#!/bin/bash
# run_after_render.sh — Wait for render pipeline to finish, then run Gemini analysis
# This script monitors the render process and kicks off analysis once complete.

set -e

echo "=== MADphotos: Post-Render Pipeline ==="
echo "Waiting for render_pipeline.py to finish..."

# Wait for render to complete
while pgrep -f "render_pipeline.py" > /dev/null 2>&1; do
...
echo "Running photography_engine.py with concurrency 5..."
