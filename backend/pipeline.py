...
def run_script(script: str, args: list, description: str) -> bool:
    ...
    cmd = [sys.executable, str(BACKEND / script)] + args
    print(f"\n{'='*60}")
    print(f"  {description}")
...
def phase_render(test: int, workers: int) -> bool:
    ...
    return run_script("render.py", args, "Phase 1: Render Originals")
...
def phase_upload() -> bool:
    ...
    if not check_gcloud_auth():
        return False
    return run_script("upload.py", ["--phase", "originals"],
...
def phase_imagen(test: int, batch_size: int, concurrent: int) -> bool:
    ...
    if test:
        args.extend(["--test", str(test)])
    return run_script("imagen.py", args,
...
def phase_render_variants(test: int, workers: int) -> bool:
    ...
    all_ok = True
    for vtype in VARIANT_TYPES:
        variant_dir = PROJECT_ROOT / "images" / "ai_variants" / vtype
...
def phase_upload_variants() -> bool:
    ...
    if not check_gcloud_auth():
        return False
    return run_script("upload.py", ["--phase", "ai_variants"],
...
def phase_finalize(conn) -> bool:
    ...
    export_path = PROJECT_ROOT / "mad_photos_export.json"
    print("Exporting database to JSON...")
    db.export_json(conn, export_path)
    print(f"  Exported to {export_path}")

    # Upload metadata
    ok = run_script("upload.py", ["--phase", "metadata"],
