...
class Checker:
    ...
    def check_gemma_ollama(self, stage: Stage) -> Dict[str, Any]:
        ...
        Matches _REQUIRED_FIELDS in run_gemma_analysis.py: 12 Gemma-unique fields
        must be non-NULL and non-empty for a row to count as complete.
        (5 Gemini-redundant fields removed: setting, time_of_day, weather,
        grading_style, faces_count — Gemini already has 9,011 coverage.)
        """
        ...
        try:
                ...
                "AND vibes IS NOT NULL AND vibes != ''"
            ).fetchone()[0]
        except sqlite3.OperationalError:
            done = 0
