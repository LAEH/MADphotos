...
DB_PATH = BASE_DIR / "mad_photos.db"
LANCE_DIR = BASE_DIR / "vectors.lance"
...
PY = sys.executable
ADV = str(BASE_DIR / "advanced_signals.py")

STAGES = [
    # --- Infrastructure ---
    Stage("Rendered Tiers",     "check_tiers",         [PY, str(BASE_DIR / "render_pipeline.py")],   group="infra"),
    Stage("Pixel Analysis",     "check_table_all",     [PY, str(BASE_DIR / "render_pipeline.py")],   group="infra"),
    Stage("EXIF Metadata",      "check_table_all",     [PY, str(BASE_DIR / "render_pipeline.py")],   group="infra"),
    ...
    # --- API models ---
    Stage("Gemini Analysis",    "check_gemini",        [PY, str(BASE_DIR / "photography_engine.py"), "--concurrent", "5"], group="model", is_api=True),

    # --- Enhancement ---
    Stage("Enhancement Plans",  "check_table_all",     [PY, str(BASE_DIR / "enhance_engine.py")],  group="signal"),
    Stage("Enhancement v2",     "check_table_all",     None,                                        group="signal"),

    # --- AI Variants ---
    Stage("AI Variants",        "check_variants",      [PY, str(BASE_DIR / "imagen_engine.py")],   group="infra", is_api=True),

    # --- GCS ---
    Stage("GCS Uploads",        "check_gcs",           [PY, str(BASE_DIR / "gcs_sync.py")],        group="infra"),
]
...
def regenerate_state() -> bool:
    ...
    script = str(BASE_DIR / "generate_status_page.py")
    try:
